...
- 深色 / 淺色主題切換
- 專案圖示上傳（R2 持久化）
- 資料庫自動備份（R2 + Google Drive 雙目標）
- 操作審計日誌（密碼重設、刪除、備份等）
- 個人化設定層（per-account user_settings：每位使用者獨立設定，持久化於 DB）
- 全站放大縮小（50–200%，SettingsTab range 控制，預設 150%，跨 session 記憶）
- 負責人彩色標籤與多人指派
- 任務狀態快速切換下拉選單
- 備份功能增強（完整導出 + 審計日誌匯出）
- Projects 卡片／明細（精簡列表）檢視切換（per-account 記憶）
- Timeline 隱藏專案（每專案眼睛 toggle，per-account）
- Timeline 收折／排序（名稱／進度／手動拖拉）
- 重新整理保持當前頁籤
