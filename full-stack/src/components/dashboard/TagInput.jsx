...
export default function TagInput({ value, onChange, suggestions = [], placeholder = "Add...", style = {}, configOwners = [] }) {
  const { X } = useTheme();
  ...
  return (<div style={{ position: "relative", ...style }}>
    <div style={{ display: "flex", flexWrap: "wrap", gap: 4, alignItems: "center", padding: "4px 8px", borderRadius: 8, border: `1px solid ${X.border}`, background: X.surface, minHeight: 32 }}>
      {tags.map((t, i) => { const oc = getOwnerColor(X, t, configOwners); return (<span key={i} style={{ display: "inline-flex", alignItems: "center", gap: 3, padding: "2px 8px", borderRadius: 12, background: oc.bg, color: oc.color, fontSize: 13, fontWeight: 500 }}>
        {t}<span onClick={() => remove(i)} style={{ cursor: "pointer", fontSize: 14, lineHeight: 1, opacity: 0.7, marginLeft: 2 }}>×</span>
      </span>); })}
      <input ref={inputRef} value={input} onChange={e => { setInput(e.target.value); setShowSug(true); }} placeholder={tags.length === 0 ? placeholder : ""} onKeyDown={e => { if ((e.key === "Enter" || e.key === ",") && input.trim()) { e.preventDefault(); add(); } if (e.key === "Backspace" && !input && tags.length > 0) remove(tags.length - 1); }} onFocus={() => setShowSug(true)} onBlur={() => setTimeout(() => setShowSug(false), 150)} style={{ border: "none", outline: "none", background: "transparent", color: X.text, fontSize: 13, flex: 1, minWidth: 60, padding: "2px 0" }} />
      {suggestions.length > 0 && <span onClick={() => { inputRef.current?.focus(); setShowSug(true); }} style={{ color: X.textDim, cursor: "pointer", fontSize: 12, padding: "0 4px", flexShrink: 0, lineHeight: 1 }}>▾</span>}
    </div>
