...
export default function Dashboard({ initialData }) {
  ...
  const defaultGH = { header: 48, projectRow: 32, taskRow: 40, taskBarHeight: 20, taskBarTop: 10 };
  const [ganttHeights, setGanttHeights] = useState(() => { try { const s = localStorage.getItem("dash-ganttHeights"); if (s) return JSON.parse(s); } catch {} return { ...defaultGH }; });
  const [ganttHeightsDraft, setGanttHeightsDraft] = useState(() => JSON.parse(JSON.stringify(ganttHeights)));
  const saveGanttHeights = () => { const filled = {}; for (const k of ["header", "projectRow", "taskRow", "taskBarHeight", "taskBarTop"]) { const val = ganttHeightsDraft[k]; filled[k] = (val === '' || val == null) ? defaultGH[k] : Math.max(1, val); } const deep = JSON.parse(JSON.stringify(filled)); setGanttHeights(deep); setGanttHeightsDraft(JSON.parse(JSON.stringify(deep))); localStorage.setItem("dash-ganttHeights", JSON.stringify(deep)); showToast("Timeline heights saved", "success"); };
  ...
  return (
    <div style={{ minHeight: "100vh", background: X.bg, fontFamily: F, color: X.text, transition: "background-color 0.3s,color 0.3s" }}>
      ...
      <div className="dash-content" style={{ maxWidth: 1400, margin: "0 auto" }}>
        ...
        {/* SETTINGS */}
        {tab === "settings" && <SettingsTab configCats={configCats} saveConfigCats={saveConfigCats} configOwners={configOwners} saveConfigOwners={saveConfigOwners} ganttDraft={ganttDraft} setGanttDraft={setGanttDraft} saveGanttWidths={saveGanttWidths} ganttHeightsDraft={ganttHeightsDraft} setGanttHeightsDraft={setGanttHeightsDraft} saveGanttHeights={saveGanttHeights} upcomingDays={upcomingDays} upcomingLimit={upcomingLimit} saveUpcomingSettings={saveUpcomingSettings} isMobile={isMobile} showToast={showToast} />}
      </div>
