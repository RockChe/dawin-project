  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 16 }}>
      ...
      <div style={{ background: X.surface, borderRadius: 12, padding: 20, border: `1px solid ${X.border}` }}>
        ...
        <div>
          <div style={{ fontSize: 13, fontWeight: 600, color: X.textSec, marginBottom: 6 }}>Upcoming Deadlines</div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            <div>
              <div style={{ fontSize: 11, color: X.textDim, marginBottom: 2 }}>Days ahead</div>
              <input type="number" value={upDaysDraft} onChange={e => setUpDaysDraft(e.target.value === '' ? '' : (parseInt(e.target.value) || 1))} onKeyDown={e => { if (e.key === "Enter") saveUpcomingSettings(upDaysDraft, upLimitDraft); }} style={{ ...iS2, fontSize: 14, padding: "6px 10px", width: "100%" }} />
            </div>
            <div>
              <div style={{ fontSize: 11, color: X.textDim, marginBottom: 2 }}>Max items</div>
              <input type="number" value={upLimitDraft} onChange={e => setUpLimitDraft(e.target.value === '' ? '' : (parseInt(e.target.value) || 1))} onKeyDown={e => { if (e.key === "Enter") saveUpcomingSettings(upDaysDraft, upLimitDraft); }} style={{ ...iS2, fontSize: 14, padding: "6px 10px", width: "100%" }} />
            </div>
          </div>
          <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 8 }}>
            <button onClick={() => saveUpcomingSettings(upDaysDraft, upLimitDraft)} style={{ background: X.accent, color: "#fff", border: "none", borderRadius: 20, padding: "7px 20px", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>Save</button>
          </div>
        </div>
      </div>
    </div>
