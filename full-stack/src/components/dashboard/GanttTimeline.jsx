...
export default function GanttTimeline({ tasks, subtasks, fp, fs, fpr, isMobile, timeDim = "月", ganttWidths, ganttHeights }) {
    ...
    <div style={{ border: `1px solid ${X.border}`, borderRadius: 12, overflow: "hidden", background: X.surface }}>
      <div style={{ display: "flex", overflow: "hidden", maxHeight: "100vh" }}>
        <div ref={lR} onScroll={() => ss(lR, rR)} className={`dash-gantt-left dash-gantt-left-collapsible${leftHidden ? " dash-gantt-left-hidden" : ""}`} style={{ overflowY: "auto", borderRight: leftHidden ? "none" : `1px solid ${X.border}`, background: X.surfaceLight }}>
          <div style={{ position: "sticky", top: 0, zIndex: 5, height: gh.header, display: "flex", alignItems: "flex-end", padding: "0 16px 10px", background: X.surfaceLight, borderBottom: `1px solid ${X.border}`, fontSize: 14, color: X.textDim }}>Project / Task</div>
          <div>{rows.map((r, i) => {
            if (r.type === "h") { const c = pcMap[r.proj] || X.accent; return (<div key={i} style={{ height: gh.projectRow, display: "flex", alignItems: "center", padding: "0 14px", gap: 8, background: `${c}10`, borderTop: i > 0 ? `1px solid ${X.border}` : "none", borderBottom: `1px solid ${c}30` }}>
            ...
            return (<div key={i} onMouseEnter={() => setHI(i)} onMouseLeave={() => setHI(null)} style={{ height: gh.taskRow, display: "flex", alignItems: "center", padding: "0 10px 0 26px", gap: 6, background: hI === i ? X.surfaceHover : "transparent", borderBottom: `1px solid ${X.border}22` }}>
        ...
        <div ref={rR} onScroll={() => ss(rR, lR)} style={{ flex: 1, overflowY: "auto", overflowX: "auto" }}>
          ...
          <div className="dash-gantt-chart" style={{ position: "relative", width: ganttMinW }}>
            ...
            {rows.map((r, i) => {
              if (r.type === "h") { const c = pcMap[r.proj] || X.accent; return (<div key={i} style={{ height: gh.projectRow, background: `${c}08`, borderTop: i > 0 ? `1px solid ${X.border}` : "none", borderBottom: `1px solid ${c}30` }} />); }
              const bc = pcMap[r.proj], hv = hI === i, dn = r.task.status === "已完成", pp = r.task.status === "提案中" || r.task.status === "待確認";
              return (<div key={i} onMouseEnter={() => setHI(i)} onMouseLeave={() => setHI(null)} style={{ position: "relative", height: gh.taskRow, background: hv ? X.surfaceHover : "transparent", zIndex: hv ? 10 : 1, borderBottom: `1px solid ${X.border}22` }}>
                <div style={{ position: "absolute", left: `${r.l}%`, width: `${r.w}%`, top: gh.taskBarTop, height: gh.taskBarHeight, borderRadius: 10, background: pp ? `repeating-linear-gradient(135deg,${bc}28,${bc}28 4px,${bc}15 4px,${bc}15 8px)` : `${bc}30`, border: `1px solid ${bc}40`, minWidth: 6 }} />
                {r.task.progress > 0 && <div style={{ position: "absolute", left: `${r.l}%`, width: `${r.w * r.task.progress / 100}%`, top: gh.taskBarTop, height: gh.taskBarHeight, borderRadius: 10, background: bc, opacity: dn ? 0.55 : 0.85, minWidth: 4 }} />}
                {r.task.progress > 0 && r.task.progress < 100 && r.w > 3 && <div style={{ position: "absolute", left: `${r.l + r.w * r.task.progress / 100 + 0.4}%`, top: gh.taskBarTop + 3, fontSize: 14, fontFamily: FM, color: bc, fontWeight: 600 }}>{r.task.progress}%</div>}
                {dn && r.w > 3 && <div style={{ position: "absolute", left: `${r.l + r.w / 2}%`, top: gh.taskBarTop + 3, fontSize: 14, fontFamily: FM, color: "#fff", textShadow: "0 1px 3px rgba(0,0,0,0.4)", fontWeight: 700, transform: "translateX(-50%)" }}>100%</div>}
