...
export default function SettingsTab({ configCats, saveConfigCats, configOwners, saveConfigOwners, ganttWidths, setGanttWidths, isMobile, showToast }) {
  const [newCat, setNewCat] = useState("");
  const [newOwner, setNewOwner] = useState("");
  const [ganttDraft, setGanttDraft] = useState(() => JSON.parse(JSON.stringify(ganttWidths)));
  const iS2 = getIS2();

  const saveGanttWidths = () => {
    const filled = {};
    for (const v of ["overview", "project", "timeline"]) {
      filled[v] = {};
      for (const k of ["day", "week", "month", "quarter"]) {
        const val = ganttDraft[v]?.[k];
        filled[v][k] = (val === '' || val == null) ? defaultGW[k] : Math.max(1, val);
      }
    }
    const deep = JSON.parse(JSON.stringify(filled));
    setGanttWidths(deep);
    setGanttDraft(JSON.parse(JSON.stringify(deep)));
