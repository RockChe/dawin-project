...
import { X, SC, PC, CC, FM, F, getIS2 } from "@/lib/theme";
...
export default function DataTableTab({
  filtered, allT, allS, allL, allF,
  projects, configCats, configOwners,
  pcMap, isMobile, userRole,
  expanded, setExpanded,
  showSubAdd, setShowSubAdd,
  subDraft, setSubDraft,
  updateTask, addTask, deleteTask,
  addSub, deleteSub, toggleSub, updateSub,
  ...
  const flatRows = useMemo(() => { const rows = []; sorted.forEach(d => { rows.push({ type: "task", id: d.id, data: d }); if (expanded.has(d.id)) { allS.filter(s => s.taskId === d.id).forEach(sub => { rows.push({ type: "sub", id: sub.id, data: sub }); }); } }); return rows; }, [sorted, expanded, allS]);
