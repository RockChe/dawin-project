    ...
    try {
      if (isNew) {
        ...
        for (const [formField, dbField] of Object.entries(fieldMap)) {
          const isDate = formField === "start" || formField === "end";
          const orig = isDate ? (task[dbField] || "") : (task[formField] || "");
          const cur = form[formField] || "";
          if (orig !== cur) {
            changes[formField] = isDate ? (cur ? toISO(cur) : null) : cur;
          }
        }
        if (form.start && form.end) {
          const dur = Math.max(1, Math.ceil((pD(form.end) - pD(form.start)) / 864e5));
          if (dur !== (task.duration || null)) changes.duration = dur;
        }
            ...
            {tFiles.map(f => {
              ...
              return (
                <div key={f.id} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6, padding: "4px 8px", borderRadius: 6, background: X.surfaceLight }}>
                  ...
                  <button onClick={async () => { try { const res = await fetch(`/api/download?key=${encodeURIComponent(f.r2Key)}`); if (!res.ok) throw new Error('Download failed'); const blob = await res.blob(); const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = f.name; a.click(); URL.revokeObjectURL(url); } catch (err) { console.error("Download failed:", err); } }} style={{ background: "transparent", border: "none", color: X.accent, fontSize: 13, cursor: "pointer", padding: "2px 6px", opacity: 0.7 }} title="下載">↓</button>
