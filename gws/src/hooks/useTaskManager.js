...
const FIELD_MAP = { task: 'task', status: 'status', category: 'category', start: 'startDate', end: 'endDate', duration: 'duration', owner: 'owner', priority: 'priority', notes: 'notes' };
...
export default function useTaskManager() {
  ...
  // updateTask(id, field, value) or updateTask(id, { field: value, ... })
  const updateTask = useCallback(async (id, fieldOrData, value) => {
    const prev = allT;
    const isBatch = typeof fieldOrData === 'object';
    if (isBatch) {
      setAllT(p => p.map(t => t.id === id ? { ...t, ...fieldOrData } : t));
      invalidateCache();
      const updateData = {};
      for (const [f, v] of Object.entries(fieldOrData)) {
        updateData[FIELD_MAP[f] || f] = v;
      }
      const result = await updateTaskAction(id, updateData);
      if (checkAuthError(result)) return;
      if (result?.error) { showToast(result.error, 'error'); setAllT(prev); }
    } else {
      setAllT(p => p.map(t => t.id === id ? { ...t, [fieldOrData]: value } : t));
      invalidateCache();
      const updateData = {};
      updateData[FIELD_MAP[fieldOrData] || fieldOrData] = value;
      const result = await updateTaskAction(id, updateData);
      if (checkAuthError(result)) return;
      if (result?.error) { showToast(result.error, 'error'); setAllT(prev); }
    }
  }, [showToast, invalidateCache, allT]);
  ...
  const deleteTask = useCallback(async (id) => {
    const prevT = allT, prevS = allS, prevL = allL, prevF = allF;
    ...
    if (checkAuthError(result)) return;
    if (result?.error) { showToast(result.error, 'error'); setAllT(prevT); setAllS(prevS); setAllL(prevL); setAllF(prevF); return; }
    showToast('任務已刪除', 'success');
  }, [showToast, invalidateCache, allT, allS, allL, allF]);
  ...
  const deleteProjectHandler = useCallback(async (id) => {
    const prevP = projects, prevT = allT;
    setProjects(p => p.filter(proj => proj.id !== id));
    setAllT(p => p.filter(t => t.projectId !== id));
    invalidateCache();
    const result = await deleteProjectAction(id);
    if (checkAuthError(result)) return;
    if (result?.error) { showToast(result.error, 'error'); setProjects(prevP); setAllT(prevT); return; }
    showToast('專案已刪除', 'success');
  }, [showToast, invalidateCache, projects, allT]);
