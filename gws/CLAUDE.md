# Dawin Dash (GWS) — 專案開發指引

## 專案概述

影視 IP 專案管理儀表板，供內部團隊追蹤多個影視專案的任務進度、時程、檔案與人員分工。

本版本使用 **Google Sheets + Google Drive**，適合小團隊或快速部署場景。

## 技術棧

- **框架**: Next.js 15.3 (App Router) + React 19
- **資料層**: Google Sheets API v4 + 自建 DAL (`sheets-dal.js`)
- **檔案儲存**: Google Drive API v3
- **快取**: 記憶體快取 (`cache.js`, 30 秒 TTL)
...
│   ├── sheets-dal.js          # Google Sheets 資料存取層（551 行，核心）
│   ├── google.js              # Google API 客戶端初始化
│   ├── drive.js               # Google Drive 檔案操作
│   ├── cache.js               # 記憶體快取（30 秒 TTL）
...
| task_status | `'已完成'`, `'進行中'`, `'待辦'`, `'提案中'`, `'待確認'` |
| priority | `'高'`, `'中'`, `'低'` |

### Tables（8 個 Google Sheet tabs，定義在 `sheets-dal.js` 的 `COLUMNS` 物件）
...
| config | 系統設定 | id, key(unique), value |
| files | 檔案記錄 | id, taskId→tasks, name, size, mimeType, r2Key（實際存 Drive 檔案 ID）, createdBy→users |
...
- `src/lib/sheets-dal.js` — Google Sheets 資料存取層（551 行，核心）
- `src/lib/google.js` — Google API 客戶端初始化
- `src/lib/drive.js` — Google Drive 檔案操作
- `src/lib/cache.js` — 記憶體快取（30 秒 TTL）
- `src/hooks/useTaskManager.js` — 核心 hook
- `src/components/dashboard/Dashboard.jsx` — 主元件（229 行）
- `scripts/seed.js` — Google Sheets 初始化腳本
...
| MCP | 用途 |
|-----|------|
| **context7** | 查詢 Next.js、Tailwind CSS 等套件的最新文件，開發時優先使用 |
...
- 踩過的坑（如 configTable 衝突、Google API 配額）
- 專案決策理由
- 外部資源位置
