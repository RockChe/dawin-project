...
│   └── api/                   # API routes (upload, download, fetch-csv, health, debug)
├── components/
│   └── dashboard/             # Dashboard 相關元件（15 個）
├── hooks/
...
- **`/api/debug` route**：目前為公開路徑，不應存在於正式環境

## 關鍵參考檔案

- `src/server/db/schema.js` — Schema 定義
...
- `src/components/dashboard/Dashboard.jsx` — 主元件（937 行）
